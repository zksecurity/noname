...
pub const BUILTIN_FN_NAMES: [&str; 4] = ["assert", "assert_eq", "log", "len"];
...
fn assert_eq_fn<B: Backend>(
    ...
    let lhs_info = &vars[0];
    let rhs_info = &vars[1];
    println!("assert_eq_fn, lhs_info: {:?}, rhs_info: {:?}", lhs_info, rhs_info);
...
fn len_fn<B: Backend>(
    ...
    dbg!("len_fn invocation");
    // we get two vars
    assert_eq!(vars.len(), 2);
    let lhs_info = &vars[0];
    let rhs_info = &vars[1];
    println!("len_fn, lhs_info: {:?}, rhs_info: {:?}", lhs_info, rhs_info);
